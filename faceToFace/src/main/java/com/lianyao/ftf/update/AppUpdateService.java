...
public class AppUpdateService {
	...
	class AutoUpgradeDelegate implements AppUpdate, ResponseCallback,
		...
		private boolean isShowPromptWhenCurrentIsLatest;
...
//					Uri.parse(latestVersion.targetUrl));
...
//			downloadTaskId = downloader.enqueue(task);
		...
		public void onCurrentIsLatest() {
			if (customShowingDelegate != null) {
				...
				if (isShowPromptWhenCurrentIsLatest) {
		...
		public void callOnResume() {
			if (isRegistered)
				return;
			isRegistered = true;
			context.registerReceiver(downloaderReceiver, new IntentFilter(
		...
		public void doIgnore() {
			if (cliListener != null) {
				cliListener.CeOkListener();
			}
		}

		@Override
		public void showPromptWhenCurrentIsLatest(boolean isShow) {
			// TODO Auto-generated method stub
			isShowPromptWhenCurrentIsLatest = isShow;
		}

		@Override
		public void onFoundLatestV(Version targetVersion) {
			onFoundLatestVersion(targetVersion);
		}

		@Override
		public void setOnCliListener(OnCliListener cliListener) {
			setOnCliListenerto(cliListener);
	...
	private AppUpdate getAppUpdate() {
		...
		return new AutoUpgradeDelegate();
	}

	public static AppUpdate getAppUpdate(Context context) {
		AppUpdateService service = new AppUpdateService(context);
		return service.getAppUpdate();
	}
	...
	private OnCliListener cliListener;

	public void setOnCliListenerto(OnCliListener cliListener) {
		this.cliListener = cliListener;
	}

	public interface OnCliListener {
