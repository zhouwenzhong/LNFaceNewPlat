...
public class CsipAccount {
	 ...
	 */
	public CsipAccount(SipProfile profile) {
		this();
		...
		cfg.setMwi_enabled(profile.mwi_enabled ? pjsuaConstants.PJ_TRUE
				: pjsuaConstants.PJ_FALSE);
	 ...
	 * @param ctxt
	 */
	public void applyExtraParams(Context ctxt11) {
		...
		if (!TextUtils.isEmpty(argument)) {
			regUri = CsipUtil.pjStrToString(cfg.getReg_uri());
			if (!TextUtils.isEmpty(regUri)) {
				long initialProxyCnt = cfg.getProxy_cnt();
				pj_str_t[] proxies = cfg.getProxy();
				String firstProxy = CsipUtil.pjStrToString(proxies[0]);
		...
		// Keep alive
		cfg.setKa_interval(30);
		// Video
		if (profile_vid_auto_show >= 0) {
			cfg.setVid_in_auto_show((profile_vid_auto_show == 1) ? pjsuaConstants.PJ_TRUE
					: pjsuaConstants.PJ_FALSE);
		} else {
			cfg.setVid_in_auto_show(pjsuaConstants.PJ_TRUE);
		}
		if (profile_vid_auto_transmit >= 0) {
			cfg.setVid_out_auto_transmit((profile_vid_auto_transmit == 1) ? pjsuaConstants.PJ_TRUE
					: pjsuaConstants.PJ_FALSE);
		} else {
	...
	public boolean equals(Object o) {
		if (o != null && o.getClass() == CsipAccount.class) {
			CsipAccount oAccount = (CsipAccount) o;
