...
package org.pjsip.api;

import java.util.regex.Pattern;

import org.pjsip.api.SipUri.ParsedSipContactInfos;
import org.pjsip.api.SipUri.ParsedSipUriInfos;
...
public class SipProfile implements Parcelable {
	 ...
	 */
	// public final static String ACCOUNT_CONTENT_TYPE =
	// SipManager.BASE_DIR_TYPE + ".account";
	/**
	 * Item type for account / sip profile
	 */
	// public final static String ACCOUNT_CONTENT_ITEM_TYPE =
	// SipManager.BASE_ITEM_TYPE + ".account";
	/**
	 * Uri of accounts / sip profiles
	 */
	// public final static Uri ACCOUNT_URI =
	 ...
	 */
	// public final static Uri ACCOUNT_ID_URI_BASE =
	// Uri.parse(ContentResolver.SCHEME_CONTENT + "://"
	// + SipManager.AUTHORITY + "/" + ACCOUNTS_TABLE_NAME + "/");
	 ...
	 */
	// public final static Uri ACCOUNT_STATUS_URI =
	// Uri.parse(ContentResolver.SCHEME_CONTENT + "://"
	 ...
	 * @see ContentUris#appendId(android.net.Uri.Builder, long)
	 */
	// public final static Uri ACCOUNT_STATUS_ID_URI_BASE =
	// Uri.parse(ContentResolver.SCHEME_CONTENT
	// + "://"
	// + SipManager.AUTHORITY + "/" + ACCOUNTS_STATUS_TABLE_NAME + "/");
	...
	/**
	 * Data useful for the wizard internal use. The format here is specific to
	 * the wizard and no assumption is made. Could be simplestring, json, base64
	 * encoded stuff etc.
	 * 
	 * @see String
	 */
	 ...
	 * >Pjsip documentation</a> 1 for true, 0 for false
	 * 
	 * @see Integer
	 */
	...
	/**
	 * Default scheme to automatically add for this account when calling without
	 * uri scheme.<br/>
	 * 
	 * This is free field but should be one of : sip, sips, tel If invalid (or
	 * empty) will automatically fallback to sip
	 */
	...
	/**
	 * If this flag is set, the authentication client framework will send an
	 * empty Authorization header in each initial request. Default is no. <a
	 * target="_blank" href=
	 ...
	 * for outgoing messages with the same interface address as the one in the
	 * REGISTER request, as long as the request uses the same transport instance
	 * as the previous REGISTER request. <br/>
	 * 
	 * Default: false <br/>
	 * <a target="_blank" href=
	 ...
	 * behavior), then the local IP address will be used. If TRUE, and when STUN
	 * and ICE are disabled, then the IP address found in registration response
	 * will be used.
	 * 
	 * Default:no
	 * 
	...
	public static final String[] LISTABLE_PROJECTION = new String[] {
			SipProfile.FIELD_ID, SipProfile.FIELD_ACC_ID,
			SipProfile.FIELD_ACTIVE, SipProfile.FIELD_DISPLAY_NAME,
			SipProfile.FIELD_WIZARD, SipProfile.FIELD_PRIORITY,
	...
	public String getDefaultDomain() {
		...
		if (parsedInfo.domain != null) {
			...
			MtcLog.d(THIS_FILE, "Domain not found for this account");
		}
		return null;
	}
